...
**Slug 唯一性大小寫口徑(Slug Uniqueness Case Sensitivity)**:
五個帶 `slug` 欄位的表分成兩種唯一性口徑。`venues`/`products`/`posts`/`courses` 四表以 `LOWER(slug)` 功能性 unique index(`uq_<table>_slug_lower`)實作,與各自 `find_by_slug` 查詢的 `WHERE LOWER(slug) = LOWER($1)` 對稱——大小寫不敏感貫穿唯一性檢查與查找兩側,四表的 create/update 皆統一由 `conflict_on_unique`/`conflict_on_constraint` 接在 repository 呼叫後轉譯 409,不再靠 SELECT 預檢(Phase 7)。`coaches` 是唯一例外:`coaches_slug_key` 是欄位級 `UNIQUE`(大小寫敏感),且 coaches 從未對外提供 slug-based 查找端點(明細端點走 UUID)——`slug` 目前只是展示用欄位,尚未真正需要案例不敏感語意。此不對稱是已知、記錄在案的落差,非 Phase 7(slug 衝突慣例統一)修復範圍,留待 Phase 11 一併處理。
_Avoid_: 假設 coaches 的 slug 衝突判斷與其他四表同樣大小寫不敏感(目前不是)。

...
`derive_status` 原本是 Rust 函式（`subscriptions::model`），`redeem_one_session` 的 `WHERE` 子句則是同一條規則的第二份獨立實作（「SQL 端 twin」）——兩者靠 `tests/service_subscriptions.rs` 的配對測試人肉守住不漂移。migration `20260708000001_subscription_derived_status.sql` 把這條規則改寫成 PostgreSQL function `subscription_derived_status(status, expires_at, remaining_sessions)`（`STABLE`，因為讀 `NOW()`；回傳 `subscription_status` enum 而非 TEXT，型別層面防拼錯），NULL 語意與原 Rust 版逐條對齊。

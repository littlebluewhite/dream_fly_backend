...
## Addendum（2026-07-20）：決策 2 的漂移風險半關——`is_revenue()` 改窮盡 match

決策 2 記錄的殘餘義務（新增 `OrderStatus` 變體時編譯器不提醒 `REVENUE_STATUSES` 同步）自此更
新：`is_revenue()` 不再讀陣列，改為無 `_` arm 窮盡 match——新增變體時此處編譯錯誤，強迫當場決定
新狀態算不算營收。常數與 7 個 SQL 綁點原樣保留；「match ↔ 陣列」漂移改由交叉測試錨定。
`VENUE_REVENUE_STATUSES` 僅 SQL 消費、無 Rust 謂詞，不加 predicate，同以交叉測試錨定。
Consequences 的人工檢查義務自此僅餘 `can_transition_to` 新邊。本檔其餘敘述維持決策當下狀態。

## Addendum（2026-07-20）：決策 9 的 `drop(tx)` 紀律收進 `TxReleased` witness

決策 9 末段（「早退分支刻意先 `drop(tx)`……」一段）同載兩樣東西：self-deadlock 的 prose 論證，
與 `drop(tx)` 這個字面呼叫形狀。W4 之後後者成為本檔第二個過時權威（第一個是決策 2 的
`REVENUE_STATUSES` 陣列，見上一則 Addendum）——「先釋放連線、再組裝回應」這條紀律的**程式面
owner** 自此遷入 `orders::service` 的私有 witness `TxReleased`（欄位私有、隔一層 `mod tx_witness`，
比照 `courses::seats` 的 `SessionLock`）。呼叫形狀隨之改變：三處 `drop(tx)` → `TxReleased::release`、
兩處 `tx.commit().await?` → `TxReleased::commit(tx).await?`，而 `assemble_response` 改收一個
`TxReleased` 參數、無 witness 不能被呼叫——invariant 從三處互指涉的註解移進型別簽章。兩處從不
開交易的讀取路徑（idempotency 預查、`get_order`）以誠實記錄在案的 `no_open_tx()` 自證斷言傳入
（該路徑無 `begin`，是登記在型別 doc 上的殘餘縫隙）。

prose 論證的權威仍在本 ADR（決策 9，及其倚賴的決策 5 鎖序骨架）；原段落敘述維持決策當下狀態、
不改寫。本 Addendum 只登記「`drop(tx)` 呼叫形狀 → `TxReleased::release`／`commit`」這一程式面遷
移，照 ADR-0002 先例（實作搬移登記於 Addendum，原決策敘述保留為當時快照）。回歸由
`tests/service_orders.rs::order_paths_complete_on_a_single_connection_pool` 固定：在 1 連線池上
循序走過 checkout／same-status no-op／狀態轉移／同 key 重放四條路徑，自我死鎖回歸會以
`PoolTimedOut` 快速失敗而非掛死。

## Addendum（2026-07-23）：決策 5 同買家 user-first 前半段收進 BalanceLock witness

決策 5 開頭「checkout 與退款現在共用同一條鎖序骨架的前半段」——`lock_balance_tx` 無條件、前移
於購物車讀取之前這段同買家論證——程式面 owner 自此遷入 `points::service` 的 `BalanceLock`
witness（欄位私有，僅 `lock_balance_tx` 能建構；無 `mod tx_witness` 隔層，比照
`courses::seats` 的 `SessionLock`——治理的呼叫端 `cart::service` 本就在別的模組，欄位私有已是
完整防線，不需要再隔一層防同檔自治理）：`lock_balance_tx` 原回傳 `i64`，改回傳
`BalanceLock`（`user_id()`/`balance()` 唯讀存取子，兩者分別是呼叫端輸入與同一次 `FOR UPDATE`
讀出）。呼叫端相應穿線：`orders::service::checkout` 續傳 `&balance_lock` 給
`cart::service::find_cart_items_for_checkout_tx`（原收裸 `user_id`，改收 `&BalanceLock`，消滅
「鎖 A 的餘額、讀 B 的購物車」這整類配對錯誤）；`compensate_order_artifacts_tx` 取鎖即棄值
（lock-only，理由見 `BalanceLock` doc 的 not-`#[must_use]` 段）。`points::service` 內部呼叫端
`try_spend_tx`／`adjust_points` 同步改讀 `.balance()`，比較邏輯與 409「點數不足」訊息字串不動。

同買家論證的散文權威隨程式碼一併遷移：`checkout` 內原本 23 行的同買家鎖序說明（無條件、前移、
鎖序骨架、死鎖環推導）壓縮為指向 `BalanceLock` doc 的數行指標，完整論證現駐 `BalanceLock` 的型
別 doc。**跨買家升序段（「Cross-buyer dimension」起始的那個註解區塊）逐字保留原地不動**——它是
`products::service`（×2）與 `cart::repository`（×1）共三處外部指涉的 anchor，搬動或改寫都會使
那些指涉失焦。本檔決策 5 段落原敘述維持決策當下狀態、不改寫，比照前兩則 Addendum 的先例（實作
搬移登記於 Addendum，原決策敘述保留為當時快照）。

**意識性排除**（本輪刻意不收 `&BalanceLock` 的四個站點，各附理由）：
- `products` 的 reserve/restore（`reserve_stock_tx`／`restore_stock_tx`）不收：耦合代價高、覆
  蓋增益近零。reserve 端——輸入的 `reserve_lines` 已是由 `cart_items`（其本身已是
  `&BalanceLock` 治理下 `find_cart_items_for_checkout_tx` 的回傳）算出的衍生值，控制流已間接
  編碼這條鏈，型別層再收一次沒有新增保障；restore 端——`compensate_order_artifacts_tx` 是單一
  40 行私有函式，`lock_balance_tx` 與 `restore_stock_tx` 的呼叫順序同一函式體內一望即知，沒有
  跨模組呼叫鏈需要型別擔保。
- `apply_delta_tx` 不收——收了就是誤述前置條件：它的正確性從不依賴先行顯式鎖（UPDATE 陳述句自
  帶列鎖），且它是「給點」的全域 owner，呼叫端含 admin 調整（`adjust_points`，呼叫前確實持鎖，
  但那是 CAS 語意的巧合，不是 `apply_delta_tx` 自身的前置條件）與 `bin/seed.rs`（在新插入列的
  同一交易內直接呼叫，沒有、也不需要先鎖）——兩類合法呼叫端不能被同一簽章一視同仁地要求持鎖。
- 退款側「orders 列鎖先於 users 列鎖」這條呼叫順序不編碼進型別：這順序只在 refund 這條路徑成
  立——`compensate_order_artifacts_tx` 由 `update_order_status` 呼叫，後者已在呼叫前用
  `FOR UPDATE` 讀鎖住 orders 列；checkout 呼叫 `lock_balance_tx` 時完全不涉及、也不持有任何
  orders 列鎖。若把「呼叫前必須已持 orders 鎖」做成型別前置條件，對 checkout 這條路徑而言就是
  簽章說謊。這條邊全程只發生在 `update_order_status` → `compensate_order_artifacts_tx` 這一個
  私有呼叫鏈內、兩次取鎖緊鄰同一函式體，散文權威（本 ADR 決策 5）加上程式碼鄰接已足以維繫。
- `cart::repository` 層保留裸 `user_id` 參數與 `pub` 可見度，不跟進收 `&BalanceLock`：回歸測試
  `checkout_cart_read_locks_products_ascending_no_cross_buyer_deadlock` 的對抗性建構刻意直呼
  repository 函式而非 service seam，證明跨買家保護（products 全域升序）獨立於 user-first 鎖序
  存在——讀端本來就不持 users 鎖。ADR-0005 seam 是既認的信任邊界，repository 對 service 的裸參
  數殘縫誠實登記於此，不強行封起來。

**回歸**：`tests/service_points.rs::concurrent_try_spend_same_user_exactly_one_succeeds`、
`tests/service_orders.rs::concurrent_checkout_last_unit_only_succeeds_once`、
`tests/service_orders.rs::checkout_cart_read_locks_products_ascending_no_cross_buyer_deadlock`、
`tests/service_orders.rs::order_paths_complete_on_a_single_connection_pool` 全數零改動、綠燈：
簽章換血後鎖拓撲、序列化行為、錯誤分類逐位元等價；「點數不足」訊息字串釘位（`service_points.rs`

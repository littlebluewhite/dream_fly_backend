...
`attendance_records`：每筆代表某場次（`course_sessions`）中、某筆報名（`enrolments`）的出勤狀態，`status` 為 `present`/`absent`/`leave` 三選一。`UNIQUE(session_id, enrolment_id)`——同一場次對同一筆報名只會有一筆紀錄，重複點名是「覆寫」而非新增一筆。**覆寫的唯一例外**：一筆由核准請假投影出的 `leave` 紀錄（該 enrolment 對此場次持有 `approved` 假單）不能被批次點名覆寫成 `present`/`absent`（核准恆勝，見裁決 5）。

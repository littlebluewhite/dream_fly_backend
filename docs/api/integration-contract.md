...
- `venue_usage`：**本月**（呼叫時先冪等物化本月場次）已物化場次 JOIN `course_schedule_slots`（`course_id`+DOW+`start_time` 可逆鍵）取 `venue`、SUM 場次分鐘數；**`venue` 為 NULL（或無對應 slot）的場次不入**。非固定桶——無場次的場地不出列。

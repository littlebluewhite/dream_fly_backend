...
2. **時間採牆鐘（wall-clock）語意，不做 timezone 轉換**：`session_date`／`start_time`／`end_time` 皆為 naive 值；本節所有「今天」的判定一律是伺服器的 **UTC 日期**（`Utc::now().date_naive()`），**不**像 `schedule`/`bookings` 模組（`POST /schedule/slots` 建立時段時的「是否已過去」檢查）那樣套用 `studio_timezone`（`Asia/Taipei`）換算。多數時段（尤其晚間常規課）UTC 與台北日期一致，但台北時間 00:00–07:59（對應 UTC 前一天 16:00–23:59）這段窗口會有一天之差——這是 v1 刻意的簡化，非 bug；若之後要嚴格對齊館所時區，需要另外調整 `today`/`from` 的計算方式。

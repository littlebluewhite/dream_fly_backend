...
`conversations`/`messages`：教練與會員之間的一對一對話。每組 `(member_id, coach_id)` 至多一筆對話（`UNIQUE(member_id, coach_id)`），由 `POST /conversations` get-or-create（配對已存在就回既有那筆，不會重複建立）。**v1 不支援檔案附件**——前端 mock 資料中的 `sharedFiles` 欄位本 API 不提供，前端須自行處理（例如隱藏該區塊或留待後續版本）。
...
回應：`ConversationSummaryResponse[]`（**純陣列，不分頁**），依 `last_message_at DESC NULLS LAST` 排序（尚無訊息的對話排最後）：

...
/// (defaults: from=today, to=from+28d). 422 if `to < from` or the span
/// exceeds `MAX_RANGE_DAYS`. 404 if the course doesn't exist.
...
/// `coaches` row, rather than erroring). Materializes today's sessions for
/// the relevant scope first, mirroring `list_course_sessions`. Role gating
